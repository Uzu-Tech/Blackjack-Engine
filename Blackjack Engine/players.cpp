...
Player::Action Player::decideAction(const Card& upcard) const
{
	...
	if (current_hand_->isPairHand()) {
		return pair_hand_lookup_[hand_value - HandConstants::MIN_PAIR_HAND][upcard_lookup];
	}
...
void Player::splitHand(Deck& deck)
{
	...
	if (has_split_) {
		throw std::runtime_error("Cannot split hand twice");
	}
		...
		hand_value++)
	{
			...
			upcard_value++)
		{
			...
			if (hand_value <= 14) {
				...
				soft_hand_lookup_[hand_lookup][upcard_lookup] = (
					upcard_value == 6 ?
					Player::Action::DOUBLE : Player::Action::STAND
					);
			}
...
{
	for (int pair_value = HandConstants::MIN_SOFT_HAND;
		pair_value <= HandConstants::MAX_SOFT_HAND;
		pair_value++)
	{
		for (int upcard_value = HandConstants::MIN_DEALER_UPCARD;
			upcard_value <= HandConstants::MAX_DEALER_UPCARD;
		...
		{
			// Shift the hand value and upcard value to match the lookup table indexes
			int pair_lookup = pair_value - HandConstants::MIN_HARD_HAND;
			int upcard_lookup = upcard_value - HandConstants::MIN_DEALER_UPCARD;
...
void Player::displayHand() const
{
	...
	if (hand_.isEmpty() && has_split_) {
		std::cout << "Player's Second Hand: ";
	}
	else {
		std::cout << "Player's Hand: ";
	}

	for (const Card& card : current_hand_->getHand()) {
		std::cout << card.getRankAsString() << " ";
	}

	std::cout << "\nHand Value: " << hand_.getHandValue() << "\n\n";
}
